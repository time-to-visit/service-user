...
func Test_RepositoryUser_RegisterUser(t *testing.T) {
	...
	repo := repository.NewRepositoryUser(gormDB)

	t.Run("Success", func(t *testing.T) {
		mock.ExpectBegin()

		mock.ExpectExec("INSERT INTO `users` (.+) VALUES (.+)").WithArgs(
			sqlmock.AnyArg(),
			sqlmock.AnyArg(),
			sqlmock.AnyArg(),
			sqlmock.AnyArg(),
		...
		result, err := repo.RegisterUser(user)
